package stateMachine
{
	...
	public class StateMachine extends EventDispatcher
	{
		...
		public var trace_info:Boolean = false;
		public var trace_errors:Boolean = true;
		...
		**/
		public function changeState(stateTo:String):void
		{
			...
			if (!(stateTo in _states)){
				if (trace_errors) trace("[StateMachine]",id,"Cannot make transition: State "+ stateTo +" is not defined");
				return;
			}
			...
			{
				if (trace_errors) trace("[StateMachine]",id,"Transition to "+ stateTo +" denied");
			...
			if (trace_info) trace("[StateMachine]",id,"State Changed to " + _state);
